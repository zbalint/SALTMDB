...
> **Status: Quick Wins implemented in v0.1.0-alpha.55** (2026-07-28). Shipped: a `UNIQUE(source_id, target_id, predicate)` index on `relations` (preceded by a one-time dedup backfill), and a new standalone `predicates` canonicalization table (mirroring tags' `canonical_id`-alias shape, seeded with `resolves`/`depends_on`/`references`/`elaborates_on`/`consolidated_from`/`supersedes`/`relates_to`, with `relates_to`/`references` pre-aliased to `elaborates_on`) via `resolve_or_create_predicate()` in `relation_service.py`, wired into `store_relation`/`bulk_store_relations` (an `ON CONFLICT DO NOTHING` no-op on duplicate edges instead of inserting a second identical row). New `get_canonical_predicates` MCP tool mirrors `get_canonical_tags`.
>
> **Status: Medium-Term implemented in v0.1.0-alpha.57** (2026-07-29, Schema Version 10). Both Medium-Term items shipped: (1) Graphiti-style `valid_to` invalidation — `commit_consolidation` now expires old relation edges in place (`valid_to = now`) and inserts replacement edges at the consolidated entity instead of mutating rows, with `consolidated_from` edges excluded from repointing to protect multi-generation lineage on re-merge; the `UNIQUE` index became partial (`WHERE valid_to IS NULL`) to allow expired+active coexistence; invalidation stayed consolidation-only (no new tool), per this doc's own recommendation. (2) `parent_ids`/`relations` redundancy resolved — `analyze_lineage` now walks `relations` (`consolidated_from` edges) via the same recursive-CTE pattern as `analyze_dependencies` instead of `entities.parent_ids` string-matching; `parent_ids` demoted to derived/display-only (kept, not dropped — the viewer reads it directly). Also added: `point_in_time` traversal on `analyze_dependencies`/`analyze_lineage`/`inspect_graph`; the viewer's independently-duplicated `get_lineage` traversal now delegates to `analyze_lineage`. Larger Bets (confidence/provenance fields, PROV-O model) remain future work — see `MIGRATION.md`'s alpha.57 entry and memory `e141bcad-4010-4169-a422-2418a7636521`.
