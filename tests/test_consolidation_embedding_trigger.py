...
class TestConsolidationEmbeddingTrigger(unittest.TestCase):
    ...
    def test_commit_consolidation_triggers_embedding_generation(self):
        res1 = store_memory(title="Parent Fact A", content="Detailed description of Fact A for testing consolidation embedding", owner_id="agent1", skip_duplicate_check=True, db_path=self.db_path)
        id1 = res1.split("ID: ")[1]

        res2 = store_memory(title="Parent Fact B", content="Detailed description of Fact B for testing consolidation embedding", owner_id="agent1", skip_duplicate_check=True, db_path=self.db_path)
        id2 = res2.split("ID: ")[1]
        ...
        ready = False
        for _ in range(50):
            row = self.conn.execute("SELECT embedding_status FROM entities WHERE id = ?", (consolidated_id,)).fetchone()
            if row and row[0] == "ready":
                ready = True
                break
            time.sleep(0.1)

        self.assertTrue(ready, "Consolidated entity's embedding status did not transition to 'ready'")
