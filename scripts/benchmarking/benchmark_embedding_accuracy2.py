...
Round 2 of the retrieval-accuracy benchmark for the memory-core rework's Foundation phase (see
plans/ and SALTMDB memory `5c09effa`) -- genuinely paraphrased queries (n=30) against real repo
docs/handover text, fixing round 1's (benchmark_embedding_accuracy.py) lexical-leakage flaw.
Baseline run (no BGE query-instruction prefix); see benchmark_embedding_accuracy3.py for the
prefix-variant comparison. Findings: chunk-size accuracy tied across all settings (SALTMDB memory
`92a3e17d`); this is the run that surfaced the BAAI/bge-small-en-v1.5 low-Top-1-accuracy finding
that motivated the prefix experiment (SALTMDB memory `9d5090dd`).
...
def run_accuracy_benchmark():
    ...
    print("\n=== CHUNKING ACCURACY BENCHMARK (Mean(Max(CosineSimilarity))) ===")
    ...
    for size, overlap in settings:
        ...
        for q_obj in queries:
            ...
            # Chunk and embed the query as well
