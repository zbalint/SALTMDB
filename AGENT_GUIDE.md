...
Agents interface with SALTMDB via **19 consolidated MCP tools** exposed by the `saltmdb` package ([tools.py](src/saltmdb/mcp/tools.py)):
...
## 2. Available Tools Overview (19 Consolidated Tools)

> [!NOTE]
> The server registers exactly **19 MCP tools**: `search_memory`, `store_memory`, `update_memory_metadata`, `get_memory`, `inspect_memory`, `revise_memory`, `supersede_memory`, `get_lineage`, `get_related_memories`, `retrieve_context`, `search_tags`, `list_predicates`, `merge_tags`, `log_event`, `get_events`, `archive_memory`, `manage_relation`, `consolidate_memories`, `review_core_memory`. Any tool count listed as "10", "12", "13", "14", "15", "16", or "18" in older references or memory is stale and incorrect — several pre-redesign tool names were renamed, merged into another tool, or removed from the MCP surface entirely in the agent API redesign, and `retrieve_context` (graph-aware, budget-bounded context assembly with `local`/`global` strategies) was added afterward, bringing the count from 18 to 19; `MIGRATION.md`'s dedicated entry for that redesign has the full old-name-to-new-name mapping if you're reconciling stale cached knowledge of this system. Two maintenance-scan capabilities that used to be MCP tools (exporting the full corpus as a snapshot; scanning for unlinked/orphaned memories) now live only in `saltmdb-cli`, not MCP — see §7's CLI notes and `saltmdb-cli --help`.
...
* `get_memory(entity_id)`: **New.** Retrieves one memory by full ID or an unambiguous ID prefix (≥8 hex chars) — the dedicated explicit-retrieval path that replaces `search_memory`'s old `entity_id`/`fetch_full` combination. Includes archived memories and returns the memory's status plus its lineage; an archived ID is never silently redirected to whatever superseded it. An ambiguous prefix (2+ matches) is rejected with the candidate list (never their content) instead of guessing. **New**: content past `CONTENT_FILE_DUMP_THRESHOLD_CHARS` (20,000 chars) is dumped to a local file (`get_content_dump_dir()`, default `~/.saltmdb/content_dumps/`, overridable via `SALTMDB_CONTENT_DUMP_DIR`) instead of inlined — the response substitutes `content_file_path` plus a 500-char `content_preview` for the `content` field. Don't assume `content` is always present in a `get_memory` response; check for `content_file_path` first and read that file when it's there.
...
* `revise_memory(entity_id, title, content, content_file_path, tags, reason, context_id, scope, memory_type, repoint_relations)`: **New.** Repairs a deficient memory representation — wrong facts, bad formatting, an incomplete write — using a brand-new immutable entity ID. `entity_id` (the target being revised) is never mutated in place: the predecessor is archived byte-for-byte exactly as it was, and the new entity links to it with a `revises` edge (new → old). An inactive (already archived/superseded) target is a hard failure reporting the known active successor(s) so you don't blindly revise stale content. The replacement is attributed to the configured caller; `context_id`/`scope`/`memory_type` are inherited when omitted. `content_file_path` behaves exactly as on `store_memory`. `repoint_relations` (default `False`) is documented under `supersede_memory` immediately below — the exact same opt-in flag, threaded through identically here.
* `supersede_memory(entity_id, title, content, content_file_path, tags, reason, context_id, scope, memory_type, repoint_relations)`: **New.** Replaces valid-but-outdated knowledge with newer knowledge — same immutable-identity shape as `revise_memory`, but links the new entity to the old with `supersedes` instead of `revises`. `supersedes` is one of the strong predicates gated by `manage_relation`'s embedding-similarity governance check (see below), so an unrelated "replacement" that doesn't actually resemble its predecessor can still be rejected. An inactive target is never silently redirected; the error reports known active successors and lineage. `content_file_path` behaves exactly as on `store_memory`; a body past the same 20,000-char dump threshold is written to a file only *after* the replacement's write transaction actually commits (a rejected write leaves no orphaned dump file), and the response's `changed` field carries `content_file_path`/`content_preview` in place of `content` when that happens. **`repoint_relations` (new, default `False`, opt-in)** — see §5 below ("Repointing semantic relations on replacement") for the full behavior; in short, setting it `True` atomically invalidates and recreates every active non-lifecycle edge touching the predecessor onto the new entity, instead of leaving them for you to walk `orphaned_semantic_edges` and repoint by hand via `manage_relation`.
...
* `retrieve_context(query, limit, budget_tokens, strategy)`: **New.** Assembles graph-aware, budget-bounded context for a query in one call — primary search hits, one-hop predicate-allowlisted graph expansion, contradiction/conflict-set surfacing, and lifecycle/supersession lineage, deterministically packed into a token budget — so you don't hand-compose `search_memory` + `get_related_memories` + `get_lineage` yourself for this common shape. Returns one `memories[]` array (each item tagged `inclusion: "primary"|"expansion"|"conflict_only"`, plus a `retrieval_provenance` explaining how it entered the result), the in-network edges directly connecting primary hits, a `lineage` map for every surfaced supersession head (contradiction-flagged where relevant), `conflict_sets` for any *unresolved* contradiction touching the result (a lifecycle-resolved conflict surfaces only via `lineage`, never here), and `metadata.fan_out`/`metadata.budget` truncation accounting (always explicit, never silent). `limit` (default 5) bounds how many primary hits seed expansion; `budget_tokens` caps the total `memories[]` token payload (server default 8,000 tokens, hard ceiling 32,000 — both set from live-usage calibration against the real corpus, not guessed). `strategy` (default `"local"`) picks the retrieval mode: `"local"` is the bounded one-hop-plus expansion just described; `"global"` instead seeds from whichever Leiden-detected community clusters (hierarchical sub-clustering; see the community-detection subsystem behind `manage_relation`'s graph) sit nearest the query by embedding-centroid similarity, and returns a representative-plus-ranked-members synthesis per seeded community — built for whole-topic breadth ("everything about X"), not one-hop-local depth. `"global"` never runs the primary-search/expansion/conflict/lineage pipeline at all: `edges`/`lineage`/`conflict_sets` are always empty, and `memories[]` items instead carry `inclusion: "community_representative"|"community_member"` with a different `retrieval_provenance` shape (community id plus a similarity score) under a new `metadata.community` namespace in place of `metadata.fan_out`. Every other traversal choice — which relation predicates expand context, how far, in which direction — is fixed for this tool, not caller-configurable.
...
### Repointing semantic relations on replacement (`repoint_relations`)

By default (`repoint_relations=False`, unchanged from before this flag existed), `revise_memory`/`supersede_memory` never touch the predecessor's own semantic relations — any `manage_relation` edge still pointing at the archived old `entity_id` after the replacement shows up in the response's `orphaned_semantic_edges` for you to repoint by hand, one at a time, via `manage_relation`. This asymmetry is deliberate, not an oversight: a lazy caller who never opts in still gets a correct-if-sparse graph (edges pointing at an archived-but-readable predecessor), never a richer-but-wrong one — the same "design for the weakest caller" reasoning `consolidate_memories` already follows for its own orphaned-edge worklist.

Pass `repoint_relations=True` to opt into the richer behavior instead: every active non-lifecycle edge touching the predecessor — both as source and as target, all 11 agent-selectable predicates, no direction or predicate filtering — is invalidated and an equivalent edge recreated onto the new entity, atomically inside the same write transaction as the replacement itself. The new edge's `created_at`/`valid_from`/`valid_at` are the operation's own current timestamp, never copied forward from the old edge — this matters, because backdating a repointed edge's validity window to before the new entity existed is exactly the kind of bitemporal fabrication safe-degradation design exists to prevent. The repoint bypasses `manage_relation`'s `RELATION_GATE_*` similarity/contradiction governance entirely: this is identity continuity for an edge you already explicitly approved once, not a new semantic claim being asserted.

Two edge cases the repoint handles by leaving the affected edge alone rather than fabricating something invalid:
- An edge that would become **self-referential** on the new entity (a pre-existing self-loop on the predecessor, a known rare anomaly) is left completely untouched — still active, still on the archived predecessor, still reported in `orphaned_semantic_edges` — rather than create a self-loop `manage_relation` itself would refuse.
- An edge whose repointed triple **collides with one that already exists** (two edges in the worklist normalizing to the same `(source, target, predicate)`, or a genuine race) has its old copy invalidated as usual, but no new relation id is reported for it in `repointed_relations` — the pre-existing edge already covers the relationship, so nothing was actually created to report.

The response always carries `semantic_relations_repointed` (echoes the flag) and `repointed_relations` (a list of `{old_relation_id, new_relation_id, source_id, target_id, predicate}` for each edge actually rewritten — empty when the flag is `False` or nothing needed repointing). `orphaned_semantic_edges` reflects the real post-write outcome, not the raw pre-transaction snapshot: with the flag `True` it lists only edges genuinely left untouched (the self-loop case above); with the flag `False` it's the full snapshot, exactly as before this flag existed.

When to reach for it: `repoint_relations=True` is a judgment call about whether this replacement is *identity-preserving continuity* — the same thing, corrected or grown, deserving every edge the old node had (e.g. revising a growing tracker/index document, or fixing a typo in an otherwise-fine memory) — versus a *correction that might deliberately invalidate what an edge asserted about the old content* (e.g. superseding a decision that was itself wrong, where a `depends_on` edge from a since-abandoned plan shouldn't just carry forward onto the new decision unexamined). Default to `False` and repoint by hand via `manage_relation` when in doubt; reach for `True` only when you're confident every existing edge still applies to the replacement as-is.
