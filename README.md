...
| `get_related_memories` | `entity_id`, `max_depth`, `direction` (`outbound`/`inbound`/`both`) | The other half of that dissolved graph-inspection tool — its old dependency-traversal mode. Multi-hop traversal of ordinary semantic relation edges from one memory. `direction` defaults to `both`, unioning independent inbound and outbound traversals. |
