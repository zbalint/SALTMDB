...
* Only a normal adapter `goodbye` writes `ended_at`, synchronously. A raw connection loss does not:
...
- **Hybrid Retrieval and Final Ranking (`search_memory`)**:
  ...
  - The agent-facing parameter surface is intentionally small: `query_keywords`, `limit`, `cursor`, `context_id`, `tags_filter`, `memory_type_filter`, `is_core`, `include_related`, `mode` — no parameter-name aliases, no reranking flags, no explicit-ID retrieval (that's `get_memory`'s job now).
