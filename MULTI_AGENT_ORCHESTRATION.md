...
* **Execution:** Before delegating a task, generate a descriptive `context_id` (e.g., `task_auth_refactor_12`). Instruct the sub-agent (in its prompt) to pass this exact string directly into the `context_id` parameter of every `log_event` and `store_memory` call it makes.
