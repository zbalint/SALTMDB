...
class TestSupersessionBugfix(unittest.TestCase):
    ...
    def test_unreviewed_weight_preservation(self):
        ...
        self.assertEqual(
            row[0], 10, "Existing memory weight should NOT be demoted automatically to 1"
        )

        # Assert no auto-created 'supersedes' relation edge exists
        rel = self.conn.execute(
            "SELECT id FROM relations WHERE predicate = 'supersedes' AND target_id = ?", (orig_id,)
        ).fetchone()
        self.assertIsNone(
            rel,
            "No 'supersedes' relation edge should be automatically created without confirmation",
        )
    ...
    def test_distinct_fact_negative(self):
        ...
        self.assertNotIn(
            ...
            fact2_res,
            "Distinct fact should NOT trigger duplicate warning",
        )
        ...
        self.assertEqual(
            len(events), 0, "Distinct fact should NOT trigger supersession_candidate event"
        )
    ...
    def test_semantic_disabled_safety(self):
        ...
        self.assertEqual(
            len(events),
            0,
            "When SALTMDB_ENABLE_SEMANTIC is false, no supersession_candidate events should be logged",
        )
