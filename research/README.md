...
| 5 | [`05-tag-system.md`](./05-tag-system.md) | Diagnoses *why* fragmentation still happens despite existing tooling: `get_canonical_tags` is advisory not enforced, and heuristic merging is lexical-only so it structurally can't catch semantic synonyms (`#fix`/`#bugfix`). Fix: make the lookup mandatory at write time, add an embedding-based candidate queue (never auto-merge), and a `facet:value` naming convention for a small fixed facet set. |
