...
6. **Look Before You Write (Deduplication & Truth Verification)**
   ...
   * If a returned candidate is genuinely the same knowledge, resolve it with `supersede_memory` (replace one memory with another) or `consolidate_memories` (merge several) rather than leaving redundant entries live.
