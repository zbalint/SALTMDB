...
**Amendment 4 (post-lock, pre-implementation adjudication, applied after Amendment 3)**: OMP reported a fourth `BLOCKED — SPEC ADJUDICATION REQUIRED` after Amendment 3, with two distinct claims, both checked directly rather than trusted from the report:

*Claim 1 (F401 on `memory_service`, rejected)*: OMP reported `ruff check` failing with `F401 'saltmdb.domain.services.memory_service' imported but unused` "after applying only §2.2," and stated the spec permits no `# noqa: F401`, no typed-only use, and no change to the mock mechanism. This is incorrect as a spec contradiction — §2.1 (above) already explicitly mandates `# noqa: F401` on exactly this import line, for exactly this reason. Verified empirically, not by re-reading prose alone: applied §2.1 and §2.2 together (imports plus this section's full replacement, plus §2.3/§2.4) to a throwaway copy of the real file in this worktree and ran the exact §9 `ruff check`/`ruff format --check` commands — `ruff check`: **All checks passed!** (zero F401, zero F841, zero anything), confirming the `# noqa: F401` genuinely suppresses this diagnostic once §2.1 is applied alongside §2.2. OMP's "after applying only §2.2" framing means it tested an intentionally incomplete, partial application of the spec (§2.2 without §2.1's own import-line change) — an expected intermediate failure state, not a real gap between two sections of the locked spec. `ruff format --check` on the same throwaway copy wanted only cosmetic line-wrapping/quote-style changes (verified via `ruff format --diff` — 100-char-width wrapping and single→double-quote normalization, zero semantic difference), exactly what running `ruff format` before finishing resolves; not a spec defect.

*Claim 2 (two basedpyright errors on the changed files)*: `query: str | None` passed to `_assemble_global_context(query: str, ...)`, and a `set[Any | float]`/`set[str | float]` inferred type passed where `find_orphan_community_matches` requires `set[str]`. Neither `basedpyright` nor its config exists anywhere in this repo (`uv run basedpyright` fails to spawn — not installed; no `[tool.basedpyright]`/`[tool.pyright]` section in `pyproject.toml`); this project's own configured, documented type checker is `mypy` (`[tool.mypy]`). Reproduced independently with the project's real tool against the same fully-applied throwaway copy: `mypy` flags **only** the first claim (`error: Argument 1 to "_assemble_global_context" has incompatible type "str | None"; expected "str"  [arg-type]`) — genuinely real, and genuinely new: the pre-change signature had `query: str` as a required non-Optional positional, so this gap did not and could not exist before this spec makes `query` optional at the signature level to also serve `entity_ids`-only `local` calls (§1's own already-locked rationale). `mypy` does **not** flag the second claim at all. Checked why directly against the pre-change source (`git show c7e977f:.../retrieve_context_service.py`, lines 82-97): the exact same mixed-value-type dict-literal shape (`{"id": hit["id"], "score": hit["score"]}` → `{hit["id"] for hit in primary_hits}`) already existed, byte-for-byte in structure, before this spec's diff — only the `"score"` value's source changed (`hit["score"]` pre-change vs. this section's `1.0` literal), not the shape basedpyright's stricter dict-value-union inference reacts to. This is pre-existing baseline imprecision in this file's already-loose `dict[str, Any]` typing throughout, unrelated to and unintroduced by this spec, out of its locked edit scope, and not gated by §9 (which requires only `ruff check`/`ruff format --check` for this file, no type checker at all) — the same class of finding as A5 Amendment 4's pre-existing `dispatch.py` mypy debt, left alone rather than fixed as a drive-by.

Fix: added one line, `assert query is not None  # narrowed by the validation block above`, immediately before the `_assemble_global_context(query, budget_tokens, conn)` call in the replacement below — `strategy` is a local parameter never reassigned between the earlier validation block and this call, so the assertion documents an invariant already proven true by that block, and both `mypy` and standard type checkers narrow `query: str | None` to `str` for every line after an `assert x is not None`. Verified: `mypy` on the fixed throwaway copy — `Success: no issues found in 1 source file`; `ruff check` on the same copy — still `All checks passed!` (the added `assert` introduces no new lint finding). The second (pre-existing, out-of-scope) basedpyright-only finding is left as-is, consistent with this spec's own already-locked "no drive-by cleanup" rule (§8).
