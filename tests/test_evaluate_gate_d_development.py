...
def _query(query_id: str, family: str, facet: str, sources: list[str] | None = None) -> dict:
    return {"id": query_id, "topic_family_id": family, "category": facet,
...
def test_metrics_use_grade_gains_family_macro_and_declared_facet_denominators():
    ...
    rows = {"q1": _row("a", "x", latency=100), "q2": _row("x", "b", latency=200),
...
def test_winner_ordering_excludes_lexical_and_honors_every_tiebreak():
    base = {"macro_positive_ndcg_at_10": 0.8, "grade2_recall_at_20": 0.8,
            "same_specific_fact_grade2_top1": 0.8, "warm_latency_p95_seconds": 1.0,
            "benchmark_failures": 0}
    metrics = {"lexical:bm25": dict(base), "dense:one": dict(base), "dense:two": dict(base)}
    bundles = {"lexical:bm25": {"cell": {"kind": "lexical"}},
               "dense:one": {"cell": {"kind": "dense"}}, "dense:two": {"cell": {"kind": "dense"}}}
    winner, evidence = select_winner(metrics, bundles)
    assert winner == "dense:one"  # lexical contender ID breaks the exact final tie.
    assert next(item for item in evidence if item["contender_id"] == "lexical:bm25")["ineligible_reason"] == "lexical_baseline"
