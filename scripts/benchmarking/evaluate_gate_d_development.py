...
from build_judging_matrix import load_bundles  # noqa: E402
...
def load_judging_matrix(path: Path, spec: Mapping[str, Any], query_ids: set[str]) -> dict[str, Any]:
    ...
    if not isinstance(pools, dict) or set(pools) != query_ids:
        raise GateDDevelopmentError(
            "JudgingMatrix pools do not cover exactly the development queries"
        )
...
def validate_bundle_rankings(  # noqa: C901, PLR0912
    ...
    for contender, bundle in bundles.items():
        ...
        if (
            bundle.get("run_id") is None
            or bundle.get("complete_query_count") != expected_query_count
        ):
        ...
        for row in rows:
            ...
            if not set(ids).issubset(matrix["pools"].get(query_id, {})):
                raise GateDDevelopmentError(
                    f"{contender}: ranked entity is absent from the JudgingMatrix pool"
                )
...
def compute_metrics(
    ...
    denominators = {
        "positive_ndcg_queries": 0,
        "grade2_recall_queries": 0,
        "same_specific_fact_queries": 0,
        "exact_sentence_queries": 0,
        "keyword_queries": 0,
    ...
    for query in queries:
        ...
        if recall is not None:
            ...
            denominators["grade2_recall_queries"] += 1
        ...
        if facet == "exact_sentence":
            exact.append(1.0 if top1_grade == 2 else 0.0)
            denominators["exact_sentence_queries"] += 1
        if facet == "keyword":
            keyword.append(1.0 if top1_grade == 2 else 0.0)
            denominators["keyword_queries"] += 1
        if facet == "strict_negative":
            strict_negative.append(1.0 if top1_grade < 1 else 0.0)
            denominators["strict_negative_queries"] += 1
...
def select_winner(
    all_metrics: Mapping[str, Mapping[str, Any]], bundles: Mapping[str, Mapping[str, Any]]
) -> tuple[str, list[dict[str, Any]]]:
    eligible = [
        contender
        for contender, metrics in all_metrics.items()
        if bundles[contender]["cell"].get("kind") != "lexical"
        and metrics["benchmark_failures"] == 0
        and metrics["warm_latency_p95_seconds"] <= 5.0
    ]
    if not eligible:
        raise GateDDevelopmentError(
            "no non-lexical contender satisfies the development eligibility gates"
        )
    ordered = sorted(
        eligible,
        key=lambda contender: (
            ...
            float(all_metrics[contender]["warm_latency_p95_seconds"]),
            contender,
        ),
    )
    evidence = []
        ...
        evidence.append(
            {
                ...
                "ineligible_reason": (
                    "lexical_baseline"
                    if baseline
                    else "benchmark_failures"
                    if metrics["benchmark_failures"]
                    else "warm_p95_exceeds_5_seconds"
                    if metrics["warm_latency_p95_seconds"] > 5.0
                    else None
                ),
                "selection_key": (
                    [
                        -metrics["macro_positive_ndcg_at_10"],
                        -metrics["grade2_recall_at_20"],
                        -metrics["same_specific_fact_grade2_top1"],
                        metrics["warm_latency_p95_seconds"],
                        contender,
                    ]
                    if eligible_now
                    else None
...
def evaluate(
    *,
    ...
    for contender in sorted(bundles):
        all_metrics[contender], denominators[contender] = compute_metrics(
            queries, relevance, rows[contender]
        )
    ...
    upstream = {
        ...
        "merged_labels": merged_fingerprint,
        "queries_dev": query_fingerprint,
        **{
            f"retrieval_run:{contender}": bundle["artifact_fingerprint"]
            for contender, bundle in bundles.items()
        },
        **{f"raw_judge:{judge}": digest for judge, digest in raw_fingerprints.items()},
    }
    metrics_artifact = sign_artifact(
        "DevelopmentMetrics",
        {
            "run_id": spec["run_id"],
            "spec_fingerprint": spec["artifact_fingerprint"],
            "required_metrics": list(REQUIRED_METRICS),
            "contender_metrics": all_metrics,
            "metric_definitions": {
                "macro_positive_ndcg_at_10": "mean of per-topic-family means of defined grade-gain (0,1,3) NDCG@10",
                "grade2_recall_at_20": "mean grade-2 recall@20 over queries with at least one judged grade-2 candidate",
                "same_specific_fact_grade2_top1": "top-1 final-grade-2 rate over queries declaring source_entity_ids",
                "exact_safety": "top-1 grade-2 rate for exact_sentence facet",
                "keyword_safety": "top-1 grade-2 rate for keyword facet",
                "strict_negative_safety": "one minus top-1 final-grade>=1 false-accept rate for strict_negative facet",
                "warm_latency": "nearest-rank p50/p95 of successful result-row latency_ms converted to seconds",
            },
            "denominators": denominators,
            "input_fingerprints": upstream,
            "development_query_ids": sorted(query["id"] for query in queries),
            "selected_winner": winner_id,
            "deterministic_ranking": ranking,
        },
    )
    judgments = sign_artifact(
        "DevelopmentJudgments",
        {
            ...
            "raw_judge_fingerprints": raw_fingerprints,
            "merged_labels_fingerprint": merged_fingerprint,
            "judging_matrix_fingerprint": matrix["artifact_fingerprint"],
        },
    )
    winner = sign_artifact(
        "DevelopmentWinner",
        {
            "run_id": spec["run_id"],
            "spec_fingerprint": spec["artifact_fingerprint"],
            "pipeline": {**bundles[winner_id]["cell"], "contender_id": winner_id},
            "development_metrics": all_metrics[winner_id],
            "upstream_fingerprints": {
                **upstream,
                "development_metrics": metrics_artifact["artifact_fingerprint"],
            },
            "development_query_ids": sorted(query["id"] for query in queries),
        },
    )
...
def _load_raw_labels(paths: Sequence[Path], merged: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if artifact_fingerprint(artifacts) != merged.get("raw_labels_fingerprint"):
        raise GateDDevelopmentError(
            "merged labels raw-label fingerprint does not bind supplied judges"
        )
    return artifacts
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--retrieval-runs-dir", type=Path, required=True)
    parser.add_argument("--raw-labels", type=Path, nargs=3, required=True)
    parser.add_argument("--out-dir", type=Path, required=True)
    parser.add_argument("--orchestrator-run-dir", type=Path)
    args = parser.parse_args(argv)
    try:
        spec = validate_bakeoff_spec(_read_json(args.spec))
        queries, query_fp = load_development_queries(args.queries_dev)
        matrix = load_judging_matrix(args.matrix, spec, {q["id"] for q in queries})
        relevance, merged_fp, unresolved = load_final_labels(args.merged_labels, matrix)
        bundles = load_bundles(args.retrieval_runs_dir, spec)
        raw = _load_raw_labels(args.raw_labels, _read_json(args.merged_labels))
        metrics, judgments, winner = evaluate(
            spec=spec,
            matrix=matrix,
            queries=queries,
            query_fingerprint=query_fp,
            relevance=relevance,
            merged_fingerprint=merged_fp,
            bundles=bundles,
            raw_labels=raw,
            unresolved=unresolved,
        )
        _write(args.out_dir / "development_metrics.json", metrics)
        _write(args.out_dir / "development_judgments.json", judgments)
        _write(args.out_dir / "development_winner.json", winner)
        if args.orchestrator_run_dir:
            orchestrator = BakeoffOrchestrator(args.orchestrator_run_dir)
            if not orchestrator.machine.checkpoint_path.exists():
                orchestrator.initialize(args.spec)
            index = sign_artifact(
                "IndexBuildReceipt",
                {
                    "run_id": spec["run_id"],
                    ...
                    "failures": [],
                    "retrieval_bundle_fingerprints": [
                        bundle["artifact_fingerprint"] for bundle in bundles.values()
                    ],
                },
            )
            orchestrator.advance(RunState.DEV_INDEXED, index)
            orchestrator.advance(RunState.DEV_RETRIEVED, next(iter(bundles.values())))
            orchestrator.advance(RunState.DEV_JUDGED, judgments)
            orchestrator.advance(RunState.DEV_WINNER_SIGNED, winner)
        print(
            json.dumps(
                {
                    "selected_winner": winner["pipeline"]["contender_id"],
                    "metrics_fingerprint": metrics["artifact_fingerprint"],
                    "winner_fingerprint": winner["artifact_fingerprint"],
                },
                sort_keys=True,
            )
        )
