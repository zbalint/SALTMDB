...
Status: Implemented on 2026-08-18 (branch `rework`), phases 1-7 of the plan's own suggested phasing (§8 "Before implementation" gate satisfied by the user's explicit handover authorization). Codex's implementation review (`plans/core_memory_bootstrap_governance_implementation_review.md`, changes-requested verdict) found 8 correctness findings (6 P1, 2 P2) plus 5 secondary mismatches; all were fixed the same day with direct regression coverage. Codex's follow-up review (`plans/core_memory_bootstrap_governance_followup_review.md`, changes-requested verdict) then found 3 further P1 gaps: (1) `enforce_overdue_boundary` compared `full_content` length only, letting a title/`core_reason`/`core_exit_condition`-only enlargement of an overdue core bypass the boundary; (2) core-producing consolidation excluded its own resolved parents from the overdue scan, letting it silently replace a sole overdue parent without going through `review_core_memory`; (3) preserved (omitted-from-the-call) `core_reason`/`core_exit_condition`/`core_review_after` were copied verbatim on update with no revalidation of the effective value, so a corrupted legacy core could be updated while staying malformed. All three were fixed the same day: (1) now compares the full canonical pessimistic rendered contribution (`render_core_entry(due=False)`) of the existing vs. prospective row; (2) `enforce_overdue_boundary`'s `exclude_ids` parameter was removed entirely -- every active core, including resolved consolidation parents, is always in scope; (3) preserved lifecycle fields are now revalidated through the same canonical validators a freshly-supplied value gets (`core_review_after` via a new `_validate_preserved_review_after` that is structurally strict but, deliberately, exempt from the future-only bound so an overdue-but-valid timestamp can still be preserved during a non-expanding repair). Each fix has direct regression coverage, including the two tests the follow-up review's findings 1/2 explicitly reversed. Full unittest suite (915 tests, up from 903 at the follow-up review's baseline), `mypy src`, and scoped `ruff check`/`format` on touched files are green; a scoped `bandit` pass shows only the same pre-existing parameterized-IN-clause medium findings already present elsewhere in `relation_service.py`. This second fix pass is awaiting a final Codex review. Phase 8, the live-data transition against the real production DB, is explicitly NOT executed -- it requires separate confirmation per its own text and mutates the user's real memory graph, and per the review's own acceptance gate must wait for a clean follow-up review first. Luna usability evaluation (§6) is also not run in this pass; ask the user before running it. Repo-wide `ruff check .`/`format --check`/`bandit -r src`/`deptry .` still carry pre-existing, unrelated findings in `scripts/benchmarking/` and elsewhere (confirmed pre-existing by Codex's review independently) -- not chased down as part of this fix pass.

Codex's third review (`plans/core_memory_bootstrap_governance_third_review.md`, one-change-requested verdict) confirmed all three follow-up fixes above, then found one further P1: both prospective-row builders (`memory_service/write.py`'s preview and authoritative in-transaction paths) sized an update with `"memory_type": memory_type or "fact"`, silently ignoring the existing row's actually-persisted type whenever a caller omitted `memory_type` on an update to a non-`fact` core -- correct for a fresh insert, wrong for an update, whose SQL write preserves the existing type via `COALESCE(?, entities.memory_type)`. Because a non-`fact` type name is longer than `"fact"` and appears twice in the rendered entry, this undercounted the prospective row's rendered contribution, letting some updates bypass both the overdue rendered-growth boundary and the hard 15,000-character capacity cap. Fixed the same day: added `core_governance_service.resolve_effective_memory_type` (a fresh insert with an omitted type resolves to `'fact'`; an update with an omitted type resolves to the existing row's own persisted type, falling back to `'fact'` only for a legacy NULL; a supplied type always wins), and both prospective-row builders in `write.py` now call it instead of the old `memory_type or "fact"` inline default. 20 new regression tests added (`TestResolveEffectiveMemoryType`, `TestEffectiveMemoryTypeGovernanceSizing` in `tests/test_core_governance_service.py`) covering the resolver directly, per-type overdue-growth rejection (`preference`/`procedure`/`decision`/`event`, one test method each), non-expanding preservation, explicit type-change sizing/persistence, a constructed near-15,000-char capacity boundary reproducing the exact false-admit/true-reject split the review demonstrated, and exact-digest-vs-prospective-digest consistency after an accepted update. Full unittest suite (929 tests, up from 915 at the third review's baseline), `mypy src`, and scoped `ruff check`/`format` on touched files are green; scoped `bandit` again shows only the same pre-existing parameterized-IN-clause medium finding. This third fix pass is awaiting a final Codex review before Phase 8 (the live-data transition) or the Luna usability evaluation may proceed -- both still require the user's own explicit go-ahead per the plan's own §8 gate.

Codex's fourth review (`plans/core_memory_bootstrap_governance_fourth_review.md`) **approved the implementation at code-review level: no P0/P1/P2 findings remain across all four review rounds.** It independently reran the former overdue-`preference` bypass reproducer and confirmed it now rejects with `RENDERED_DELTA 0` and zero state change. Focused core/API/memory-type suite: 227 tests OK; full suite: 929 tests OK; `mypy src` clean; scoped `ruff check`/`format --check` clean on 15 touched files. The review-series closure list (12 items) is fully resolved. This closes the code-review track entirely -- no further Codex fix round is required based on the reviewed state.

Remaining non-code rollout gates before live rollout (per the fourth review, none yet executed, all still require the user's own explicit go-ahead): (1) run the planned cold Luna usability evaluation against the final MCP schemas/descriptions; (2) resolve any critical weaker-model misunderstanding it finds; (3) commit the implementation and tests as a coherent descriptive commit (still fully uncommitted in the working tree as of this review); (4) back up the live SALTMDB database through the supported mechanism; (5) perform the documented agent-driven core-memory cleanup using SALTMDB tools only, never direct SQL; (6) verify the final live core set is at most five entries, every entry at most 2,500 content characters, and the exact rendered digest at most 15,000 characters; (7) start a fresh agent session and confirm complete injection with no hook spill/truncation warning. The live-data transition (Phase 8) still requires separate user authorization -- this review does not authorize or perform it.
