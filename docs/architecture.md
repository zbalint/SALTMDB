...
* **`communities`**: Leiden community-detection clusters computed over the explicit relation graph. Columns: `id`, `representative_entity_id` (a PageRank+cosine-medoid pick computed once at clustering time), `member_count`, `level` (`0` = leaf; `parent_community_id` self-FK supports recursive sub-clustering of oversized flat communities), `created_at`. No bitemporal columns -- a full-recompute-on-write-trigger policy makes row presence itself mean "member as of last recompute"; `community_id` is not stable across recomputes.
* **`community_membership`**: `entity_id` (PK) → `community_id`/`level` assignment, one row per clustered entity.
* **`community_embeddings`**: A `sqlite-vec` `vec0` virtual table storing each community's own centroid embedding (384-dim), mirroring `entity_embeddings`.
...
### 8. Graph-Aware Context Retrieval (`retrieve_context`)
The 19th MCP tool assembles a budget-bounded memory context around a known anchor in one call, replacing manual composition of `get_related_memories` + `get_lineage` + conflict-checking. It does not search on its own — find the memory first with `search_memory`, then hand its id here.

* **`strategy="local"` (default)**: takes a required `entity_ids: list[str]` anchor (one or more memory IDs the caller already has). Assembles one-hop, predicate-allowlisted graph expansion (`CONTEXT_EXPANSION_TOP_K_RELATIONSHIPS=10`; `contradicts` is force-included under its own separate `CONTEXT_EXPANSION_CONTRADICTS_CAP=5`), contradiction/conflict-set surfacing, and lifecycle/supersession lineage into one `memories[]` array, each item tagged `inclusion` (`primary`/`expansion`/`conflict_only`) plus a `retrieval_provenance` explaining how it entered the result, under a deterministic real-token-count budget (`CONTEXT_BUDGET_DEFAULT_TOKENS=8000`, `CONTEXT_BUDGET_MAX_TOKENS=32000`). An earlier revision seeded `local` from a `query` string via an internal `search_memory(mode="strict")` call, which silently starved results behind that same strict-mode abstention gate; the current `entity_ids`-anchor design resolves that by requiring the caller to have already resolved an anchor via `search_memory` first.
* **`strategy="global"`**: takes a required `query` string instead. Seeds from the nearest Leiden-detected community clusters by query-embedding-vs-centroid cosine similarity (`CONTEXT_GLOBAL_TOP_K_COMMUNITIES=10`, then floored by a relative-gap filter, `CONTEXT_GLOBAL_SEED_SIMILARITY_GAP=0.10`, that can only shrink the K-cap window — it is not an absolute relevance floor, so the single best-ranked community is always admitted regardless of its own absolute similarity) and returns a representative-plus-ranked-members synthesis per seeded community (`CONTEXT_GLOBAL_REPRESENTATIVE_RESERVE_CAP=5`, gated by its own `CONTEXT_GLOBAL_REPRESENTATIVE_SIMILARITY_GAP=0.10` relative-gap floor; member pool capped at `CONTEXT_GLOBAL_MEMBER_POOL_CAP=30`) — whole-topic breadth rather than one-hop-local depth. Items are tagged `community_representative`/`community_member`; `edges`/`lineage`/`conflict_sets` are always empty for this strategy.
* **Community detection** (`community_detection_service.py`) runs Leiden clustering over the explicit relation graph — never an LLM/summarization model, consistent with SALTMDB's standing no-embedded-generation-model design. It runs as a write-triggered, cooldown-throttled background pass (`COMMUNITY_DETECTION_TRIGGER_COOLDOWN_S=300`, reusing the Librarian's existing trigger-pool/cooldown mechanism — see "Single-Owner Backend Daemon & Librarian Throttling" above), never lazily at query time. Zero-edge "orphan" memories are instead assigned to their nearest existing community live at query time by embedding proximity (`COMMUNITY_ORPHAN_SIMILARITY_THRESHOLD=0.60`), never persisted. Oversized flat communities (over `COMMUNITY_HIERARCHY_SIZE_THRESHOLD=20` members) get recursively sub-clustered up to `COMMUNITY_HIERARCHY_MAX_DEPTH=2` levels.
* **Response shape**: unlike every other SALTMDB tool, a successful `retrieve_context` call does not use the `{"status": "ok", "data": ...}` envelope — it returns its own bespoke top-level shape directly (`{"entity_ids"|"query", "memories", "edges", "lineage", "conflict_sets", "metadata"}`); only a caller-input rejection uses the standard envelope. The tool's own MCP description is the authoritative contract, including two caveats not repeated here: the local-strategy budget packer is not anchor-aware (a caller-supplied primary anchor can be dropped entirely under a tight budget), and `edges` may reference an entity_id absent from the returned `memories[]`.
* **Dependency**: `leidenalg` + `igraph` (both GPL-licensed; compatible with SALTMDB's own AGPL-3.0-only license).
