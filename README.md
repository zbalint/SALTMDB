...
| `revise_memory` | `entity_id`, `title`, `content`, `tags`, `reason`, `context_id`, `scope`, `memory_type` | **New.** Repairs a deficient memory representation via a brand-new immutable entity — the predecessor is archived byte-for-byte and linked `new --revises--> old`. An inactive target is a hard failure reporting known active successors. The replacement is attributed to the configured caller; omitted `context_id`/`scope`/`memory_type` are inherited. |
