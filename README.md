...
| `log_event` | `event_type`, `content`, `context_id`, `error_code`, `owner_id` | Appends a scrubbed entry to the immutable short-term ledger. **No more `agent_id`/`agent_session_id` params** — the bound `owner_id` (first-call-wins per-session identity) is injected as the stored event's `agent_id`, `agent_session_id` is auto-populated from the adapter process session; parameter is `event_type`, not `type` (shadowed the Python builtin), with no remaining name aliases. |
