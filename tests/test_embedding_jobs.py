...
class EmbeddingJobsTests(unittest.TestCase):
    def setUp(self):
        ...
        self.conn.execute(
            ...
            "VALUES ('e','2020','2020','2020','raw','Title','Body',?)",
    ...
    def test_enqueue_is_atomic_and_replaces_old_source(self):
        ...
        rows = self.conn.execute(
            "SELECT job_kind,source_hash,state FROM embedding_jobs ORDER BY job_kind"
        ).fetchall()
        self.assertEqual(
            rows,
            [
                ("chunk", content_hash, "queued"),
                ("entity", entity_source_hash("Title", "Body"), "queued"),
            ],
        )
        enqueue_embedding_jobs_for_entity(self.conn, "e", "New", "Body", content_hash)
        self.assertEqual(
            self.conn.execute(
                "SELECT COUNT(*) FROM embedding_jobs WHERE state='cancelled'"
            ).fetchone()[0],
            1,
        )
        self.assertEqual(
            self.conn.execute("SELECT embedding_status FROM entities WHERE id='e'").fetchone()[0],
            "pending",
        )
    ...
    def test_claim_is_unique_then_expired_fifth_lease_becomes_terminal(self):
        enqueue_embedding_jobs_for_entity(
            self.conn, "e", "Title", "Body", compute_content_hash("Body")
        )
        ...
        )  # the other kind is independently claimable
        ...
        self.assertEqual(
            self.conn.execute(
                "SELECT state FROM embedding_jobs WHERE id=?", (first["id"],)
            ).fetchone()[0],
            "failed",
        )
    ...
    def test_retry_backoff_and_terminal_failure(self):
        ...
        state, attempts, due, error = self.conn.execute(
            "SELECT state,attempt_count,next_attempt_at,last_error FROM embedding_jobs WHERE id=?",
            (snapshot["id"],),
        ).fetchone()
        self.assertEqual((state, attempts, error), ("retry_wait", 1, "temporary"))
        self.assertIsNotNone(due)
        self.conn.execute(
            "UPDATE embedding_jobs SET state='running',attempt_count=5 WHERE id=?",
            (snapshot["id"],),
        )
        _retry_embedding_job(self.conn, snapshot["id"], "terminal")
        self.assertEqual(
            self.conn.execute(
                "SELECT state FROM embedding_jobs WHERE id=?", (snapshot["id"],)
            ).fetchone()[0],
            "failed",
        )
    ...
    def test_stale_result_cannot_overwrite_current_source(self):
        ...
        self.assertEqual(
            self.conn.execute(
                "SELECT COUNT(*) FROM entity_embeddings WHERE entity_id='e'"
            ).fetchone()[0],
            0,
        )
    ...
    def test_committed_jobs_survive_pre_dispatch_crash_boundary(self):
        ...
        self.assertEqual(
            reopened.execute("SELECT COUNT(*) FROM embedding_jobs WHERE state='queued'").fetchone()[
                0
            ],
            2,
        )
