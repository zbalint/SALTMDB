...
def build_judge_packets(queries: list[dict], matrix: dict, judge: str, split: str, base_seed: int = 0) -> tuple[dict, dict]:
    ...
    tasks, mapping = [], {}
    for query_id, candidates in matrix["pools"].items():
        query = query_by_id[query_id]
        rendered = [
            {"candidate_id": candidate_id, "title": item.get("title") or "", "snippet": item.get("snippet") or ""}
            for candidate_id, item in candidates.items()
        ]
        random.Random(_seed(base_seed, split, judge, query_id)).shuffle(rendered)
        task_id = f"{split}:{judge}:{query_id}"
    ...
    packet = {"schema_version": 1, "judge": judge, "split": split, "tasks": tasks}
...
def validate_labels(response: dict, private_mapping: dict, judge: str) -> list[dict]:
    ...
    for label in response.get("labels", []):
        ...
        seen.add(key)
        normalized.append({"judge": judge, "query_id": expected[task_id]["query_id"], "candidate_id": candidate_id, "grade": grade})
...
def write_packets(queries_path: Path, matrix_path: Path, out_dir: Path, judge: str, split: str, base_seed: int = 0) -> None:
    ...
    (out_dir / f"judge_packet_{judge}_{split}.json").write_text(json.dumps(packet, indent=2, ensure_ascii=False))
    (out_dir / f"judge_mapping_{judge}_{split}.json").write_text(json.dumps(private, indent=2, ensure_ascii=False))
...
def main() -> None:
    ...
    if args.action == "packets":
        write_packets(args.queries, args.matrix, args.out_dir, args.judge, args.split, args.seed)
    else:
