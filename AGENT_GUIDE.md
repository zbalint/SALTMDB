...
* `store_memory(content, tags, owner_id, scope, weight, is_core, title, entity_id, metadata, context_id, skip_duplicate_check, relevance, impact, novelty, actionability)`: Save/upsert long-term knowledge. Requires non-empty `content` and `title`. Enforces sub-millisecond Tier 1/2 quality gates (Word N-gram 3-gram/5-gram repetition & TTR bounds), Markdown syntax integrity & MSDI density scoring, SHA-256 exact hash collision checks, and Tier 4 technical quality scoring ($0.0 - 1.0$) before ONNX vector embedding execution. Supports parameter aliases (`text`, `tag`, `owner`).
