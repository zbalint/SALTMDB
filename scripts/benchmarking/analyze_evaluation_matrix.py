...
from eval_stats import (FamilyMetricSample, cluster_bootstrap_delta_ci,
                        cluster_bootstrap_mean_ci, false_accept_rate_flag, holm_adjust,
                        known_answer_recall_at_10, mcnemar_continuity_corrected,
                        misleading_top1_flag, mrr, ndcg_at_10, pooled_recall_at_10,
                        top1_direct_relevance_flag, win_loss_tie_counts)
...
def _with_ci(values: dict[str, list[float]], n_resamples: int) -> dict:
    ...
    return {"value": None if math.isnan(point) else point,
            "ci95": [None if math.isnan(low) else low, None if math.isnan(high) else high]}


def _validate_inputs(queries: list[dict], matrix: dict, relevance: dict[str, dict[str, int]], split: str) -> list[str]:
    ...
    for query in queries:
        ...
        if query_id in seen or query_id not in rankings or query_id not in pools or query_id not in relevance:
...
def analyze(queries: list[dict], matrix: dict, merged_labels: list[dict] | dict,
            n_resamples: int = 10000, split: str = "dev") -> dict:
    ...
    rows = {name: {metric: {} for metric in ("ndcg_at_10", "mrr", "pooled_recall_at_10",
                                               "known_answer_recall_at_10", "top1_direct_relevance",
                                               "misleading_top1", "false_accept_rate")} for name in configs}
        ...
        for name in configs:
            ...
            if not is_negative:
                metric_values.update({
                    ...
                    "known_answer_recall_at_10": known_answer_recall_at_10(ranking, query.get("source_entity_ids", [])),
            ...
            for metric, value in metric_values.items():
                if value is not None and (metric != "false_accept_rate" or is_negative):
                    ...
                    if category in HIGH_VALUE_CATEGORIES:
                        slices[name].setdefault(category, {}).setdefault(metric, {}).setdefault(family, []).append(value)
    metrics = {name: {metric: _with_ci(values, n_resamples) for metric, values in row.items()}
               for name, row in rows.items()}
    high_value = {name: {category: {metric: _with_ci(values, n_resamples) for metric, values in category_values.items()}
                         for category, category_values in config_values.items()}
                  for name, config_values in slices.items()}
    return {"split": split, "metrics": metrics, "high_value_slices": high_value,
            "top1_vectors": top1_vectors, "top1_query_ids": query_ids,
            "metric_families": {name: rows[name]["ndcg_at_10"] for name in configs},
            "input_hashes": {"queries": _hash(queries), "matrix": _hash(matrix),
...
def freeze_dev_contenders(analysis: dict) -> dict:
    ...
    ranked = sorted((name for name in metrics if name != CURRENT_DEFAULT_CONFIG_NAME),
                    key=lambda name: (-(metrics[name]["top1_direct_relevance"]["value"] or -1),
                                      -(metrics[name]["ndcg_at_10"]["value"] or -1), name))
    if len(ranked) < 3:
        raise ValueError("need three non-default configurations")
    contenders = ranked[:3]
    shortlist = {"schema_version": 1, "current_default": CURRENT_DEFAULT_CONFIG_NAME, "contenders": contenders,
                 "comparisons": [[contenders[0], CURRENT_DEFAULT_CONFIG_NAME],
                                 [contenders[1], CURRENT_DEFAULT_CONFIG_NAME],
                                 [contenders[2], CURRENT_DEFAULT_CONFIG_NAME], [contenders[0], contenders[1]]],
...
def validate_frozen_shortlist(shortlist: dict) -> None:
    expected = {"schema_version", "current_default", "contenders", "comparisons", "development_input_hashes", "fingerprint"}
    ...
    if len(shortlist["contenders"]) != 3 or len(set(shortlist["contenders"])) != 3 or len(shortlist["comparisons"]) != 4:
        raise ValueError("malformed frozen development shortlist")
    copy = dict(shortlist)
...
def paired_comparisons(analysis: dict, comparisons: list[list[str]], n_resamples: int = 10000) -> list[dict]:
    ...
    for contender, baseline in comparisons:
        ...
        delta, low, high = cluster_bootstrap_delta_ci(analysis["metric_families"][contender],
                                                       analysis["metric_families"][baseline], n_resamples=n_resamples)
        result.append({"contender": contender, "baseline": baseline, "top1_win_loss_tie": [wins, losses, ties],
                       "mcnemar_chi2": stat, "mcnemar_p": pvalue, "ndcg_delta": delta,
...
def blind_decision(analysis: dict, comparisons: list[dict], shortlist: dict) -> dict:
    ...
    for item in comparisons:
        ...
        if delta >= 0.04 and item["ndcg_delta_ci95"][0] is not None and item["ndcg_delta_ci95"][0] > 0:
            winners.append((delta, item["contender"]))
    if not winners:
        return {"outcome": "retain_current_default", "winner": default,
                "reason": "no frozen contender met the 4pp, Holm, and paired-NDCG criteria"}
    return {"outcome": "select_contender", "winner": max(winners)[1],
...
def main() -> None:
    ...
    analysis = analyze(_load_queries(args.queries), json.loads(args.matrix.read_text()),
                       json.loads(args.labels.read_text()), args.resamples, args.split)
    ...
    if args.split == "dev":
        ...
        result.update({"dev_contenders": shortlist, "comparisons": comparisons,
