...
* [ ] **Changed a public MCP tool's name, parameters, or return shape** (anything in `src/saltmdb/mcp/tools.py`) → update that tool's own `@mcp.tool(description=...)` text so it still matches the code. There is no second file to keep in sync anymore: `AGENT_GUIDE.md` was removed (see the note below) precisely so the docstring is the only place this documentation lives.

> [!NOTE]
> **Repo-root orchestration templates retired (`v0.1.0-alpha.74`).** `ORCHESTRATOR.md`, `MULTI_AGENT_ORCHESTRATION.md`, and `WORKER_TEMPLATE.md` used to exist as a vendor-neutral reference copy of the multi-agent orchestration protocol at the repo root, alongside a Claude-Code-specific implementation bundled at `~/.claude/skills/saltmdb-subagent-orchestration/references/`. Maintaining two independently-drifting copies (see the 2026-07-25 audit referenced above, and `MIGRATION.md` alpha.49–.51) turned out not to be worth it, so the repo-root copies were deleted; the bundled skill copy is now the sole reference implementation. `AGENT_GUIDE.md` §1 commandment 14 and §6 step 3 were updated accordingly at the time — if you change the orchestration protocol, only the skill bundle needs updating, and there's no longer a "keep the repo root vendor-neutral" constraint to reconcile against. `AGENT_GUIDE.md` itself was later removed entirely (docs-restructuring effort, see `MIGRATION.md`); this note stays for historical context only.
